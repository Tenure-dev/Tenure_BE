...
@Schema(description = "보낸 거래 의사 목록 응답")
public record PurchaseIntentSentListResponse(
        List<Item> content,
        Cursor nextCursor,
        boolean hasNext
) {
    ...
    public static PurchaseIntentSentListResponse of(
        ...
        if (hasNext && !intents.isEmpty()) {
            Item last = content.get(content.size() - 1);
            nextCursor = new Cursor(last.createdAt(), last.intentId());
        }
        return new PurchaseIntentSentListResponse(content, nextCursor, hasNext);
    }
